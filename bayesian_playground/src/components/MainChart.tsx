...
const MainChart: FC<MainChartProps> = ({ stepIndex, bayesianData }) => {
  ...
  useEffect(() => {
    ...
    if (bayesianData.length > 0) {
      ...
      const fullX = Array.isArray(cumulativeTrace.x) ? (cumulativeTrace.x as number[]) : [];
      const fullY = Array.isArray(cumulativeTrace.y) ? (cumulativeTrace.y as number[]) : [];
  ...
  return (
    <ErrorBoundary>
      <Paper sx={{ padding: 2 }}>
        ...
        <Plot
          data={plotData}
          layout={{ width: 600, height: 400, title: "True Function & Bayesian Estimate" }}
        />
      </Paper>
    </ErrorBoundary>
  );
