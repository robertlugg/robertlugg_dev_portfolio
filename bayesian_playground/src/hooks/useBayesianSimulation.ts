...
export const useBayesianSimulation = (
  acquisitionFn: string,
  initialSteps: number = 20,
) => {
    ...
    const runSimulation = async () => {
      for (let step = sampledXRef.current.length; step < totalSteps; step++) {
        if (step === 0) {
          ...
          setLogs((prev) => [
            ...prev,
            "Step 0: initial state, no sample added.\n",
          ]);
          continue;
        }
        let nextX: number;
        ...
        if (step === 1) {
          ...
          const { mean, stdDev } = gpRegression(
            sampledXRef.current,
            sampledYRef.current,
            X_PRED,
          );
          ...
          rmsError = Math.sqrt(
            mean.reduce(
              (sum, m, i) => sum + (m - trueFunction(X_PRED[i])) ** 2,
              0,
            ) / mean.length,
          );
        }
